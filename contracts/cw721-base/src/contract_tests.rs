...
use cosmwasm_std::{coins, from_binary, to_binary, CosmosMsg, DepsMut, Empty, Response, WasmMsg};
...
fn proper_instantiation() {
    ...
    let res = contract.denom(deps.as_ref()).unwrap();
...
fn minting() {
    ...
    // user can't mint
    let not_allowed = mock_info("arbitrary", &coins(1, "wrongdollar"));
    
    let err = contract
        .execute(deps.as_mut(), mock_env(), not_allowed, mint_msg.clone())
        .unwrap_err();
    
    match err {
        ContractError::Payment(_) => {},
        _ => panic!("Should have thrown the payment error"),
    }
    
    // user can mint
    let allowed = mock_info("arbitrary", &coins(1, DENOM));
    let _ = contract
